...
class LocalStorage {
  ...
  static Future<void> saveCarId({required String id}) async {
    final box = GetStorage();

    await box.write(carId, id);
  }

  static Future<void> saveCurrencyCode({required String code}) async {
    final box = GetStorage();

    await box.write(currencyCode, code);
  }

  static String getCurrencyCode() {
    return GetStorage().read(currencyCode) ?? '';
  }

  static Future<void> saveSignUpToken({required String token}) async {
    final box = GetStorage();

    await box.write(signUpToken, token);
  }

  static String getSignUpToken() {
    return GetStorage().read(signUpToken) ?? '';
  }

  static Future<void> saveCarName({required String id}) async {
    final box = GetStorage();

    await box.write(carName, id);
  }

  static Future<void> saveCarNumber({required String id}) async {
    final box = GetStorage();

    await box.write(carNumber, id);
  }

  static Future<void> saveCarFees({required String id}) async {
    final box = GetStorage();

    await box.write(carFees, id);
  }

  static String getCarId() {
    return GetStorage().read(carId);
  }

  static String getCarFees() {
    return GetStorage().read(carFees);
  }

  static String getCarName() {
    return GetStorage().read(carNumber);
  }

  static String getCarNumber() {
    return GetStorage().read(carName);
  }
