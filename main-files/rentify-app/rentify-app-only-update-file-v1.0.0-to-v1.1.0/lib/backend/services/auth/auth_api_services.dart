...
class AuthApiServices {
  ...
  ///* Forgot Password Process Api
  static Future<ForgotPasswordModel?> resendOtpProcess(
      {required String token}) async {
    Map<String, dynamic>? mapResponse;
    try {
      mapResponse = await ApiMethod(isBasic: true).get(
        "${ApiEndpoint.forgotPasswordReSendOtpURL}$token&lang=${GetStorage().read('selectedLanguage')}",
        code: 200,
      );
      if (mapResponse != null) {
        ForgotPasswordModel result = ForgotPasswordModel.fromJson(mapResponse);
  ...
  static Future<CommonSuccessModel?> emailResendProcessApi(
    ...
    try {
      mapResponse = await ApiMethod(isBasic: false).get(
        "${ApiEndpoint.registerOtpResendURL}$token&lang=${GetStorage().read('selectedLanguage')}",
        code: 200,
      );
      if (mapResponse != null) {
        CommonSuccessModel result = CommonSuccessModel.fromJson(mapResponse);
